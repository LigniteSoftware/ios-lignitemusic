...
-(instancetype) initWithButtons:(NSArray*) buttonsArray direction:(MGSwipeDirection) direction differentWidth:(BOOL) differentWidth buttonsDistance:(CGFloat) buttonsDistance
{
    ...
    if (self = [super initWithFrame:CGRectMake(0, 0, containerWidth, maxSize.height)]) {
		...
		//Add rounded corners
		_container.layer.cornerRadius = 8.0f;
...
-(void) createSwipeViewIfNeeded
{
    ...
    if (!_leftView && _leftButtons.count > 0) {
        ...
        _leftView.cell = self;
        _leftView.frame = CGRectMake(-_leftView.bounds.size.width - 5, _leftSwipeSettings.topMargin, _leftView.bounds.size.width - 5, _swipeOverlay.bounds.size.height - _leftSwipeSettings.topMargin - _leftSwipeSettings.bottomMargin);
    ...
    if (!_rightView && _rightButtons.count > 0) {
        ...
        _rightView.frame = CGRectMake(_swipeOverlay.bounds.size.width + 5, _rightSwipeSettings.topMargin, _rightView.bounds.size.width + 5, _swipeOverlay.bounds.size.height - _rightSwipeSettings.topMargin - _rightSwipeSettings.bottomMargin);
